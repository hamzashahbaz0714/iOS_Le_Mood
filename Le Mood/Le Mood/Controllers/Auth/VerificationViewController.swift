        ...
        AuthService.instance.registerUserWithEmail(withEmail: passUser?.email ?? "", andPassword: password ?? "") { [self] (success, error) in
            if success {
                AuthService.instance.loginUserWithEmail(email: passUser?.email ?? "", password: password ?? "") { (succes, error) in
                    if success {
                        let user = UserModel(id: Auth.auth().currentUser!.uid, name: passUser?.name ?? "", nikName: passUser?.nikName ?? "", email: passUser?.email ?? "", phoneNumber: passUser?.phoneNumber ?? "", image: passUser?.image ?? "", gender: passUser?.gender ?? "", deviceType: passUser?.deviceType ?? "ios", country: passUser?.country ?? "", region: passUser?.region ?? "",moodId: "",moodType: "",moodValue: 0,lastMoodDate: "",fcmToken: "", language: "", isMoodVisible: true)
