        ...
        if txtNumber.text != "" {
            ...
            PhoneAuthProvider.provider().verifyPhoneNumber("\(txtNumber.text!.replacingOccurrences(of: " ", with: ""))", uiDelegate: nil) { [self] (verificationID, error) in
                if error == nil {
                    ...
                    let user = UserModel(id: Auth.auth().currentUser?.uid ?? "", name: passUser?.name ?? "", email: passUser?.email ?? "", phoneNumber: "\(txtNumber.text!)", image: passUser?.image ?? "", gender: passUser?.gender ?? "", country: passUser?.country ?? "", region: passUser?.region ?? "",moodId: "",moodType: "",moodValue: 0,lastMoodDate: "",fcmToken: "")
