        ...
        if txtNumber.text != "" {
            ...
            PhoneAuthProvider.provider().verifyPhoneNumber("\(txtNumber.text!.replacingOccurrences(of: " ", with: ""))", uiDelegate: nil) { [self] (verificationID, error) in
                if error == nil {
                    ...
                    let user = UserModel(id: Auth.auth().currentUser?.uid ?? "", name: passUser?.name ?? "", nikName: passUser?.nikName ?? "", email: passUser?.email ?? "", phoneNumber: "\(txtNumber.text!)", image: passUser?.image ?? "", gender: passUser?.gender ?? "", deviceType: "ios", country: passUser?.country ?? "", region: passUser?.region ?? "",moodId: "",moodType: "",moodValue: 0,lastMoodDate: "",fcmToken: "", language: "", isMoodVisible: true)
