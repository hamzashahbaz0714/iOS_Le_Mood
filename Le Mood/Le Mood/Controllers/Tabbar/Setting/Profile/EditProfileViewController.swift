...
class EditProfileViewController: UIViewController {
    ...
    func imagePickerController(_ picker: UIImagePickerController, didFinishPickingMediaWithInfo info: [UIImagePickerController.InfoKey : Any]) {
        ...
        DataService.instance.uploadProfilePicture(image: image) { [self](success, url) in
            if success {
                ProgressHUD.dismiss()
                let updateUser = UserModel(id: user?.id ?? "", name: user?.name ?? "", nikName: user?.nikName ?? "", email: user?.email ?? "", phoneNumber: user?.phoneNumber ?? "", image: url, gender: user?.gender ?? "", country: user?.country ?? "", region: user?.region ?? "",moodId: user?.moodId ?? "",moodType: user?.moodType ?? "",moodValue: user?.moodValue ?? 0,lastMoodDate: "",fcmToken: "", language: user?.language ?? "", isMoodVisible: user?.isMoodVisible ?? true)
    ...
    func updateUserProfile(){
        if txtName.text != "" {
            ProgressHUD.show()
            let updateUser = UserModel(id: user?.id ?? "", name: txtName.text!, nikName: user?.nikName ?? "", email: user?.email ?? "", phoneNumber: user?.phoneNumber ?? "", image: user?.image ?? "", gender: user?.gender ?? "", country: user?.country ?? "", region: user?.region ?? "",moodId: user?.moodId ?? "",moodType: user?.moodType ?? "",moodValue: user?.moodValue ?? 0,lastMoodDate: "",fcmToken: "", language: user?.language ?? "", isMoodVisible: user?.isMoodVisible ?? true)
            DataService.instance.updateUser(user: updateUser)
            DataService.instance.setCurrentUser(user: updateUser)
            txtName.text = ""
            ProgressHUD.dismiss()
