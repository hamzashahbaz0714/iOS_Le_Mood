...
class EditProfileViewController: UIViewController {
    ...
    func imagePickerController(_ picker: UIImagePickerController, didFinishPickingMediaWithInfo info: [UIImagePickerController.InfoKey : Any]) {
        ...
        DataService.instance.uploadProfilePicture(image: image) { [self](success, url) in
            if success {
                ProgressHUD.dismiss()
                let updateUser = UserModel(id: user?.id ?? "", name: user?.name ?? "", email: user?.email ?? "", phoneNumber: user?.phoneNumber ?? "", image: url, gender: user?.gender ?? "", country: user?.country ?? "", region: user?.region ?? "")
    ...
    func updateUserProfile(){
        if txtName.text != "" {
            ProgressHUD.show()
            let updateUser = UserModel(id: user?.id ?? "", name: txtName.text!, email: user?.email ?? "", phoneNumber: user?.phoneNumber ?? "", image: user?.image ?? "", gender: user?.gender ?? "", country: user?.country ?? "", region: user?.region ?? "")
            DataService.instance.updateUser(user: updateUser)
            DataService.instance.setCurrentUser(user: updateUser)
            txtName.text = ""
            ProgressHUD.dismiss()
